...
function PoliticsArticleCard({ article, isLarge = false }: { article: any, isLarge?: boolean }) {
  const { imageUrl, isLoading } = useNewsImage(article.title, 'Politics');
...
export default function PoliticsPage() {
  ...
  const politicsArticles = news.politics || [];
  const featuredArticle = politicsArticles[0];
  const sidebarArticles = politicsArticles.slice(1, 5);
  const remainingArticles = politicsArticles.slice(5);
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <h1 className="text-6xl font-bold mb-12 text-black">Politics</h1>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-yellow-700">
                  <strong>Limited Content:</strong> Showing curated political news while we refresh our live feed.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2">
            {featuredArticle && (
              <PoliticsArticleCard article={featuredArticle} isLarge={true} />
            )}
            
            {/* Additional Articles */}
            <div className="border-t border-gray-200 pt-8">
              {remainingArticles.map((article, index) => (
                <PoliticsArticleCard key={article.id || index} article={article} />
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="border-t-4 border-black pt-6">
              <h3 className="text-2xl font-bold mb-6 text-black">Latest Political News</h3>
